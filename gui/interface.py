...
class MainScreen(BoxLayout):
    ...
    def __init__(self, welcome=False, **kwargs):
        ...
        if welcome:
            welcome_popup = Popup(title='Coursera ML in Python', content=Label(
                text='Coursera Assignment App'), size_hint=(1, 1))
    ...
    def titlebar(self):
        ...
        self.submit_popup = Popup(
            title='Enter credentials', content=self.accept_credentials(), size_hint=(0.6, 0.35))
    ...
    def console(self):
        ...
        self.info_label = TextInput(size_hint=(1, None), readonly=True, background_color=(
            0, 0, 0, 1), foreground_color=(1, 1, 1, 1), opacity=0)
    ...
    def run(self, instance):
        ...
        command = ["python", "../" + self.current_ex +
                   "/" + self.current_ex + ".py"]
    ...
    def saveAssignment(self, assignment, *largs):
        ...
        try:
            if not self.element.readFile(self.current_ex, self.current_file) == assignment.text:
                filehandler = self.element.writeFile(
                    self.current_ex, self.current_file)
    ...
    def schedule_reload(self, instance, value):
        if value:
            ...
            # TODO:When clicking on another file, both focus=False and filebar
            # button callbacks are executed simultaneously leading to deadlock
    ...
    def filebar(self):
        ...
        for f in files:
            if f.strip() == self.current_file:
                button = ToggleButton(
                    text=f, group=self.current_ex, state='down')
            else:
                button = ToggleButton(
                    text=f, group=self.current_ex, state='normal')
            button.width = 200
            button.size_hint = (None, 1)
            button.bind(on_press=self.update_currentFile)
            layout.add_widget(button)

        filebar = ScrollView(size_hint=(1, None),
